...
class Labyrinth:
    ...
    def is_free(self, position):  # проверка на стенку
        return self.get_tile_id(position) in self.free_tiles

    def find_path_step(self, start, target):
        INF = 1000  # расстояние от стартовой клетки
        x, y = start
        distance = [[INF] * self.width for _ in range(self.height)]
        distance[x][y] = 0  # расстояние до стартовой клетки
        prev = [[None] * self.width for _ in range(self.height)]
        queue = [(x, y)]  # очедерь клетки
        while queue:
            x, y = queue.pop(0)
            for dx, dy in (1, 0), (0, 1), (-1, 0), (0, -1):  # рассматриваем соседние клетки
...
class Enemy:
    ...
    def __init__(self,  position):
        self.x, self.y = position
        self.delay = 100  # задержка срабатывания таймера
...
# вывод win/lose
# def show_nessage(sreen, message):
    ...
    # pygame.draw.rect(sreen, (208, 150, 50), (text_x - 10, text_y - 10,
                                             # text_w + 20, text_h + 20))
    # sreen.blit(text, (text_x, text_y))
...
def main():
    ...
    while running:
        ...
        # if game.check_win():
            # game_over = True
            # show_nessage(screen, "You won!")
        # if game.check_lose():
            # game_over = True
            # show_nessage(screen, "You lost!")
        pygame.display.flip()
        clock.tick(FPS)
    pygame.quit()
